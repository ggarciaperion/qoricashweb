    ...
    return (
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: '#0D1117' }}>{account.bank_name}</div>
          ...
          {account.origen && (
            <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>{account.origen}</div>
          )}
        </div>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        <div style={{
          ...
          <span style={{ fontSize: 13, fontWeight: 800, color: '#0D1117' }}>{symbol}</span>
        </div>
        <span style={{ fontWeight: 700, fontSize: 14, color: '#0D1117' }}>{label}</span>
        <span style={{ fontSize: 11, color: '#6B7280', background: 'rgba(0,0,0,0.06)', borderRadius: 20, padding: '2px 8px', fontWeight: 600 }}>
          {list.length}
        </span>
      </div>
      {list.length === 0 ? (
    ...
    <>
      <div className="px-4 sm:px-6 py-5 sm:py-7 max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5 sm:mb-7">
          <div>
            <h1 className="text-lg sm:text-xl font-extrabold m-0" style={{ color: '#0D1117' }}>Cuentas Bancarias</h1>
            <p className="text-xs sm:text-sm mt-1" style={{ color: '#6B7280' }}>Administra tus cuentas registradas</p>
          </div>
          <button
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            <div style={{
              ...
              <h3 style={{ fontSize: 17, fontWeight: 800, color: '#0D1117', margin: '0 0 8px' }}>¿Eliminar cuenta?</h3>
              ...
              <p style={{ fontSize: 12, color: '#6B7280', margin: '0 0 4px', fontFamily: 'monospace' }}>
                {deleteTarget.account.account_number}
              </p>
              <p style={{ fontSize: 12, color: '#9CA3AF', margin: '0 0 24px' }}>Esta acción no se puede deshacer.</p>
              {deleteError && (
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  disabled={isDeleting}
                  style={{ flex: 1, padding: '11px', borderRadius: 10, border: '1px solid rgba(0,0,0,0.12)', background: '#F8FAFC', fontSize: 13, fontWeight: 600, color: '#374151', cursor: 'pointer' }}
                >
                  Cancelar
                </button>
                <button
